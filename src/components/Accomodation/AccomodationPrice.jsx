...
export default function AccomodationPrice({ bgUrl }) {
  ...
  return (
    <>
          ...
          <section className="space-y-6">
            ...
            <article className=" flex flex-col md:flex-row items-center ">
              <div className="md:w-2/3 p-4">
                <h3 className="text-lg  font-semibold teramo text-coklat_tua tracking-wider">
                ...
                <div className=" ml-7 border border-coklat_tua inline-block rounded-md px-4 py-2 text-coklat_tua font-bold">
                  <Link href="/">IDR 500.000 / Malam</Link>
                </div>
                <section className=" mt-5 ">
                  <button
                    onClick={() => setShow(true)}
                    className="w-full bg-coklat_tua inline-block rounded-md px-4 py-2 text-white font-bold text-center cursor-pointer"
                  >
                    Chat Sekarang
                  </button>
                </section>
      ...
      {show && <FormPemesanan setShowPopup={setShow} />}
    </>
  );
}
