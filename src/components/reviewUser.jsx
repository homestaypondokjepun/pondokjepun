...
import reviews from "../components/data/reviews.json";
...
const ReviewUser = () => {
    ...
    const [expandedReviews, setExpandedReviews] = useState({}); // simpan state expand per review
    const step = 5; // jumlah tambahan tiap klik
            ...
            <section className="w-full max-w-5xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                    {reviews.slice(0, visibleCount).map((review) => {
                        ...
                        const maxLength = 80; // karakter awal
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">
                                            {review.review_date_time
                                                ? new Date(review.review_date_time).toLocaleDateString()
                ...
                {reviews.length > 5 && (
                    <div className="text-center mt-4 flex gap-2 justify-center">
                        {visibleCount < reviews.length && (
                            <button
                                onClick={showMore}
                                className="px-4 py-2 bg-coklat_tua text-white rounded-lg cursor-pointer hover:bg-coklat_muda hover:text-coklat_tua "
                            >
                                Show More
                            </button>
                        )}
                        {visibleCount > 5 && (
                            <button
                                onClick={showLess}
                                className="px-4 py-2 bg-coklat_muda text-coklat_tua rounded-lg cursor-pointer hover:bg-coklat_tua hover:text-white"
                            >
                                Show Less
                            </button>
                        )}
